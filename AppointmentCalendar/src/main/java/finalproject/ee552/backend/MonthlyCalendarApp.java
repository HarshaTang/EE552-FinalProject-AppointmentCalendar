...
public class MonthlyCalendarApp {
	...
	private Map<Integer, String> generateDayMonthMap() {
		Map<Integer, String> tempDayMonthMap = new HashMap<>();
		
		int maxLengthOfMonth = YearMonth.of(this.year, this.month).lengthOfMonth();
		
		for (int i = 1; i <= maxLengthOfMonth; i++) {
			tempDayMonthMap.put(i, getDayOfWeek(i));
		}
		
		return tempDayMonthMap;
	}
	...
	private List<Map<String, Integer>> generateFullCalendarLayout() {
		List<Map<String, Integer>> fullCalendarMapList = new ArrayList<>();
		...
		Map<String, Integer> weekFormatMap = new HashMap<> ();
		...
		Map<String, Integer> tempWeekFormatMap = new HashMap<>();
