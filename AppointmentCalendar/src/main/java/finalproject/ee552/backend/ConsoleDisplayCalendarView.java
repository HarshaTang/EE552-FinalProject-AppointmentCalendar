...
public class ConsoleDisplayCalendarView {
	...
	public String generateWeeklyView(int startDay) {
		...
		if (startDay <= maxDays) {
			...
			return genView(newStart, newEnd);
		} else {
			return ("Cannot display invalid date! The last day your requested month is "+maxDays+"!");
		}
	...
	private String genView (int startDay, int endDay) {
		...
		List<String> timeSlotsKeys = new ArrayList<>(scheduler.getTimeSlotsMap(endDay).keySet());
